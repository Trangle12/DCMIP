...
from clustercontrast.trainers import ClusterContrastTrainer, ClusterContrastTrainer_PCLMP
from clustercontrast.evaluators import Evaluator, extract_features
from clustercontrast.utils.data import IterLoader
from clustercontrast.utils.data import transforms as T
from clustercontrast.utils.data.preprocessor import Preprocessor
from clustercontrast.utils.logging import Logger
from clustercontrast.utils.serialization import load_checkpoint, save_checkpoint_stage1, \
    save_checkpoint_stage2
...
def main_worker_stage1(args):
    ...
    # Create datasets
    iters = args.iters_stage1
    print("==> Load unlabeled dataset")
    dataset = get_data(args.dataset, args.data_dir)
    test_loader = get_test_loader(dataset, args.height, args.width, args.batch_size, args.workers)

    # Create model
    model = create_model_stage1(args)
    ...
    for epoch in range(args.epochs_stage1):
        ...
        memory = ClusterMemory(model.module.num_features, num_cluster, temp=args.temp,
                                   momentum=args.momentum, mode=args.memorybank,
        ...
        if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs_stage1 - 1):
            mAP = evaluator.evaluate(test_loader, dataset.query, dataset.gallery, cmc_flag=False)
            is_best = (mAP > best_mAP)
            best_mAP = max(mAP, best_mAP)
            save_checkpoint_stage1({
...
def main_worker_stage2(args):
    ...
    checkpoint = load_checkpoint(osp.join(args.logs_dir, 'model_best_stage1.pth.tar'))
    ...
    for epoch in range(args.epochs_stage2):
        ...
        memory_features = generate_random_features(pseudo_labels, features_ema, num_cluster, args.num_instances)

        del cluster_loader, features

        # Create hybrid memory
        memory = ClusterMemory(model.module.num_features, num_cluster, temp=args.temp,momentum=args.momentum, mode='CMhard',num_instances=args.num_instances).cuda()

        # Cluster proxies
        memory.features = F.normalize(cluster_features.repeat(2, 1), dim=1).cuda()
        # Instance proxies
        memory.features_ema = F.normalize(memory_features, dim=1).cuda()
        ...
        if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs_stage2 - 1):
    ...
    checkpoint = load_checkpoint(osp.join(args.logs_dir, 'model_best_stage2.pth.tar'))
    model.load_state_dict(checkpoint['state_dict'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-mb', '--memorybank', type=str, default='CMhybrid',
    ...
    parser.add_argument('--epochs_stage1', type=int, default=40)
    parser.add_argument('--epochs_stage2', type=int, default=50)
    parser.add_argument('--iters_stage1', type=int, default=100)
    parser.add_argument('--iters_stage2', type=int, default=100)
