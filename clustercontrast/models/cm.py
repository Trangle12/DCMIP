...
class CM_Hybrid(autograd.Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        for index, features in batch_centers.items():
            ...
            # ctx.features[index] = ctx.features[index] * ctx.momentum + (1 - ctx.momentum) * features[hard]
            # ctx.features[index] /= ctx.features[index].norm()

            #MSMT17
            for feature in features:
                ctx.features[index] = ctx.features[index] * ctx.momentum + (1 - ctx.momentum) * feature
                ctx.features[index] /= ctx.features[index].norm()
...
class ClusterMemory(nn.Module, ABC):
    def __init__(self, num_features, num_samples, temp=0.05, momentum=0.2, mode='CMhybrid', hard_weight=0.5, smooth=0,
                 num_instances=16):
    ...
    def forward(self, inputs, targets, model_name='encoder'):
        ...
        if self.cm_type == 'CM':
            ...
            if model_name == 'encoder':
                ...
                raise NotImplementedError('CMHard is not implemented.')
